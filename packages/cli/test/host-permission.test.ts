...
test("hostCommandPermissionAction maps commands to team permission actions", () => {
  ...
  assert.equal(
    hostCommandPermissionAction("workflow-update", { id: "decision-1", status: "done" }),
    "approve-decision",
  );
...
test("hostCommandPermissionAction honors an explicit, valid permissionAction override", () => {
  ...
  assert.equal(
    hostCommandPermissionAction("task-update", { status: "in_progress", permissionAction: "change-scope" }),
    "change-scope",
  );
  // An unknown action is ignored and inference applies.
  assert.equal(
    hostCommandPermissionAction("task-update", { status: "done", permissionAction: "not-a-real-action" }),
    "close-task",
  );
});
...
test("runHostCommand denies a role-forbidden command end to end", async () => {
  const result = await runHostCommand(
    { command: "usage", actorRole: "builder" },
    {
...
test("runHostCommand escalates a human-decision command into a decision card and audit trail", async () => {
  ...
  const decision = (blocked.json as { decision?: { id?: string; kind?: string; status?: string; ownerRole?: string } })
    .decision;
  ...
  assert.equal(
    (cards.json as { cards?: Array<{ id?: string }> }).cards?.some((card) => card.id === decision?.id),
    true,
  );
