...
interface CardJson {
  card?: { id?: string; ownerRole?: string; status?: string };
  handoffs?: Array<{ reason?: string; card?: { id?: string; kind?: string; ownerRole?: string; status?: string } }>;
}
...
test("workflow-update auto-routes a completed card to its reviewer", async () => {
  ...
  assert.equal(
    (listed.json as { cards?: Array<{ kind?: string }> }).cards?.some((card) => card.kind === "review"),
    true,
  );
});
...
test("workflow-update rejects done when dependencies or acceptance evidence are missing", async () => {
  ...
  await assert.rejects(
    runHostCommand({
      command: "workflow-update",
      input: { outputDir, id: "main", status: "done", result: "implemented" },
    }),
...
test("review completion requires a verdict and routes changes back to the source owner", async () => {
  ...
  await assert.rejects(
    runHostCommand({
      command: "workflow-update",
      input: { outputDir, id: reviewCardId, status: "done", result: "looks fine" },
    }),
...
test("compact-ledger requires confirmation and rewrites the merged ledger", async () => {
  ...
  await runHostCommand(
    {
      command: "workflow-create",
      input: { outputDir, kind: "task", id: "c1", title: "Card one", ownerRole: "builder" },
    },
    deps,
  );
