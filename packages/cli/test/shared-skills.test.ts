...
import {
  ...
  listSharedSkills,
  sharedSkillRoots,
  sharedSkillSnapshotsRoot,
} from "../src/shared-skills.js";

test("sharedSkillRoots reads KING skill roots", () => {
  assert.deepEqual(sharedSkillRoots({ KING_AI_SHARED_SKILLS: `/a${delimiter}/b,/c` } as NodeJS.ProcessEnv), [
    "/a",
    "/b",
    "/c",
  ]);
...
test("sharedSkillSnapshotsRoot reads KING snapshot roots", () => {
  assert.equal(
    sharedSkillSnapshotsRoot({ KING_AI_SKILL_SNAPSHOTS_DIR: "/snapshots" } as NodeJS.ProcessEnv),
    "/snapshots",
  );
...
test("installSharedSkills copies shared skills into Claude and Codex homes", async () => {
  ...
  try {
    ...
    const result = await installSharedSkills(home, [root], {
      KING_AI_SKILL_SNAPSHOTS_DIR: snapshotsRoot,
    } as NodeJS.ProcessEnv);

    assert.deepEqual(
      result.installed.map((skill) => skill.name),
      ["takeover-context"],
    );
    ...
    const manifest = JSON.parse(await readFile(result.snapshot.manifestPath, "utf8")) as {
    ...
    assert.equal(
      await readFile(join(home, ".claude", "skills", "takeover-context", "SKILL.md"), "utf8"),
      "# Takeover\n",
    );
