...
describe("classifyCelebritySearchSnapshot", () => {
  ...
  it("detects login and challenge states", () => {
    assert.equal(
      classifyCelebritySearchSnapshot("elonmusk", { text: "Sign in to X\nPhone, email, or username" }).status,
      "auth-required",
    );
    assert.equal(
      classifyCelebritySearchSnapshot("elonmusk", { text: "Verify you are human to continue" }).status,
      "challenge",
    );
  });
});
