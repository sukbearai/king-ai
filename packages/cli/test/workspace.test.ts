...
test("resolveWorkspaceAllowlist parses KING_WORKSPACES with path delimiter and commas", () => {
  const paths = resolveWorkspaceAllowlist({
    KING_WORKSPACES: [`~/workspace/github`, "/tmp/one,/tmp/two", "/tmp/one"].join(delimiter)
...
test("agentWorkspaceRoot uses configured per-agent base or private home workspace", () => {
  ...
  assert.equal(
    agentWorkspaceRoot("demo-agent", "/tmp/home/demo-agent", { KING_AGENT_WORKSPACE_ROOT: "/tmp/agent-worktrees" } as NodeJS.ProcessEnv),
    resolve("/tmp/agent-worktrees/demo-agent")
  );
  assert.equal(detectLocalCapabilities({ KING_AGENT_WORKSPACE_ROOT: "/tmp/agent-worktrees" } as NodeJS.ProcessEnv).agentWorkspaceRoot, resolve("/tmp/agent-worktrees"));
});
