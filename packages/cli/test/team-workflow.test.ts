...
import {
  KING_AI_ROLE_TEMPLATES,
  checkTeamPermission,
  defaultTeamSpec,
  hasExplicitImplementationIntent,
  isPlannerGuidanceText,
  normalizeTeamRoleId,
  requiredCapabilitiesForText,
  roleTemplateForAgent,
  scenarioTemplate,
} from "../src/team-workflow.js";

test("default team exposes structured role templates and routing policy", () => {
  const team = defaultTeamSpec();
  assert.deepEqual(
    KING_AI_ROLE_TEMPLATES.map((role) => role.id),
    ["planner", "builder", "reviewer", "tester", "ops", "researcher", "doc-writer", "summarizer"],
  );
  ...
  assert.equal(
    team.permissionPolicy.rules.some((rule) => rule.role === "ops" && rule.allow.includes("deploy-release")),
    true,
  );
  assert.equal(team.roles.find((role) => role.template === "builder")?.handoffPolicy.reviewerRole, "reviewer");
  assert.equal(
    team.roles.every((role) => role.responsibility.length > 20),
    true,
  );
  ...
  assert.equal(
    team.roles.some((role) => role.template === "summarizer"),
    true,
  );
  assert.equal(
    team.permissionPolicy.rules.some((rule) => rule.role === "summarizer"),
    true,
  );
});
...
test("shared collaboration semantics classify agents and request capabilities", () => {
  ...
  assert.equal(
    roleTemplateForAgent({ id: "custom", name: "QA", role: "Role template: tester. Verify releases." }),
    "tester",
  );
  assert.equal(
    roleTemplateForAgent({ id: "writer", name: "Writer", role: "Documentation and release notes" }),
    "doc-writer",
  );
  ...
  assert.deepEqual(requiredCapabilitiesForText("prepare release approval and audit queue"), [
    "ops",
    "release",
    "audit",
  ]);
...
test("built-in team scenarios include roles tasks and acceptance", () => {
  ...
  for (const id of ids) {
    ...
    assert.equal(
      scenario.tasks.every((task) => task.ownerRole && task.acceptance.length > 0),
      true,
    );
    assert.equal(
      scenario.tasks.some((task) => task.reviewerRole),
      true,
    );
  }
});
