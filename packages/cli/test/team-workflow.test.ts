...
import { KING_ROLE_TEMPLATES, checkTeamPermission, defaultTeamSpec, normalizeTeamRoleId, requiredCapabilitiesForText, roleTemplateForAgent, scenarioTemplate } from "../src/team-workflow.js";

test("default team exposes structured role templates and routing policy", () => {
  const team = defaultTeamSpec();
  assert.deepEqual(KING_ROLE_TEMPLATES.map((role) => role.id), ["planner", "builder", "reviewer", "tester", "ops", "researcher", "doc-writer", "summarizer"]);
  ...
  assert.equal(team.roles.length, KING_ROLE_TEMPLATES.length);
...
test("team permission policy maps role actions to runtime decisions", () => {
  ...
  assert.equal(normalizeTeamRoleId("king-ceo"), "planner");
...
test("shared collaboration semantics classify agents and request capabilities", () => {
  assert.equal(roleTemplateForAgent({ id: "king-ceo", name: "King CEO", role: "Coordinate work" }), "planner");
