...
test("host server exposes remote test device configuration endpoints", async (t) => {
    ...
    runCommand: async (request) => {
      ...
      if (request.command === "remote-config-get") {
        return {
          ...
          exitCode: 0,
          text: "config",
          json: { config: { defaultDevice: devices[0]?.id, devices }, devices },
        };
      }
  ...
  const saved = (await fetch(`${baseUrl}/remote/devices`, {
    ...
    ok?: boolean;
    command?: string;
    json?: { device?: { id?: string; password?: string; auth?: string } };
  };
  ...
  const listed = (await fetch(`${baseUrl}/remote/devices`).then((res) => res.json())) as {
    ok?: boolean;
    json?: { devices?: Array<{ id?: string; password?: string }> };
  };
  assert.equal(listed.ok, true);
  assert.equal(listed.json?.devices?.[0]?.id, "test-61");
  assert.equal(listed.json?.devices?.[0]?.password, undefined);

  const removed = (await fetch(`${baseUrl}/remote/devices/test-61`, { method: "DELETE" }).then((res) =>
    res.json(),
  )) as { ok?: boolean; command?: string; json?: { devices?: unknown[] } };
...
test("createHostStatusServer serves read-only app endpoints", async (t) => {
  ...
  const health = (await fetch(`http://127.0.0.1:${port}/health`).then((res) => res.json())) as {
    ok?: boolean;
    readOnly?: boolean;
  };
  ...
  const status = (await fetch(`http://127.0.0.1:${port}/status`).then((res) => res.json())) as {
    ok?: boolean;
    computerId?: string;
  };
  ...
  const events = (await fetch(`http://127.0.0.1:${port}/events`).then((res) => res.json())) as {
    events?: Array<{ kind: string }>;
  };
  assert.equal(events.events?.[0]?.kind, "agent.started");

  const streamController = new AbortController();
  const stream = await fetch(`http://127.0.0.1:${port}/status/stream?interval=1000`, {
  ...
  const hostStream = await fetch(`http://127.0.0.1:${port}/host/stream?interval=1000&limit=1`, {
    signal: hostStreamController.signal,
  });
  ...
  const timelineStream = await fetch(`http://127.0.0.1:${port}/timeline/stream?interval=1000&limit=1`, {
    signal: timelineStreamController.signal,
  });
  ...
  assert.deepEqual(
    commands.commands?.filter((entry) => entry.destructive).map((entry) => entry.name),
  ...
  assert.equal(
    timeline.json?.events?.some((event) => event.command === "status"),
    true,
  );
  ...
  const planRunResource = (await fetch(`http://127.0.0.1:${port}/runs/plan`, {
    ...
    ok?: boolean;
    command?: string;
    json?: { spec?: { goal?: string }; options?: { engine?: string } };
  };
  ...
  const preflightResource = (await fetch(`http://127.0.0.1:${port}/runs/preflight`, {
    ...
    ok?: boolean;
    command?: string;
    json?: { spec?: { goal?: string }; ready?: boolean };
  };
...
test("createHostStatusServer exposes app run request REST endpoints", async (t) => {
  ...
  await writeFile(
    join(eventsOutput, "loop-events.ndjson"),
    [
      JSON.stringify({
  ...
  await writeFile(
    join(eventsOutput, ".king-ai", "heartbeat.json"),
    JSON.stringify({
  ...
  const submitted = (await fetch(`${baseUrl}/runs`, {
    ...
    body: JSON.stringify({
      goal: "rest queued run",
      requestId: "rest-1",
      options: { engine: "codex", outputDir: eventsOutput },
    }),
  }).then((res) => res.json())) as {
    ok?: boolean;
    command?: string;
  ...
  const listed = (await fetch(`${baseUrl}/runs?limit=5`).then((res) => res.json())) as {
    ok?: boolean;
    json?: { requests?: Array<{ id: string }> };
  };
  assert.equal(listed.ok, true);
  ...
  const singleRunStream = await fetch(`${baseUrl}/runs/rest-1/stream?interval=1000`, {
  ...
  const single = (await fetch(`${baseUrl}/runs/rest-1`).then((res) => res.json())) as {
    ok?: boolean;
    json?: { request?: { status?: string } };
  };
  ...
  const emittedRunEvent = (await fetch(`${baseUrl}/runs/rest-1/events`, {
    ...
    command?: string;
    json?: {
      event?: { type?: string; runId?: string; source?: string; message?: string; payload?: { panel?: string } };
    };
  };
  ...
  const appEvents = (await fetch(`${baseUrl}/runs/rest-1/events?type=app.note&writeResults=false`).then((res) =>
    res.json(),
  )) as {
    ok?: boolean;
  ...
  const updated = (await fetch(`${baseUrl}/runs/rest-1`, {
    ...
    ok?: boolean;
    command?: string;
    json?: { request?: { status?: string; detail?: string } };
  };
  ...
  const statusEvents = (await fetch(`${baseUrl}/runs/rest-1/events?type=run.status&writeResults=false`).then((res) =>
    res.json(),
  )) as {
    ok?: boolean;
    command?: string;
  ...
  const executed = (await fetch(`${baseUrl}/runs/rest-exec-1/execute`, { method: "POST" }).then((res) =>
    res.json(),
  )) as {
    ok?: boolean;
    command?: string;
...
test("createHostStatusServer exposes app export REST endpoints with policy", async (t) => {
  ...
  const plan = (await fetch(`${baseUrl}/exports/plan`, {
    ...
    ok?: boolean;
    command?: string;
    json?: { runId?: string; workspaceFileCount?: number };
  };
