...
test("taskDoneTransition centralizes GUI and host task review transitions", () => {
  assert.deepEqual(
    taskDoneTransition({ id: "task-1", kind: "task", status: "assigned", assignee: "dev" }, "dev", {
  ...
  assert.deepEqual(
    taskDoneTransition({ id: "task-1", kind: "task", status: "review", assignee: "reviewer" }, "reviewer", {
      coordinatorId: "king-ai-ceo",
      reviewerId: "reviewer",
      hasConversation: true,
    }),
