...
test("normalizeRuntimeAttachments accepts images and rejects required invalid files", () => {
  ...
  assert.deepEqual(requiredAttachmentsRejected(attachments).map((item) => item.name), ["binary.exe"]);
...
test("normalizeRuntimeAttachments accepts readable text and source files", () => {
  const attachments = normalizeRuntimeAttachments([
    ...
    { name: "fallback.html", mime: "application/octet-stream", size: 80, url: "https://example.com/fallback.html", required: true },
...
test("cacheLocalAttachments copies accepted file attachments into media cache", async () => {
  ...
  const [attachment] = await cacheLocalAttachments(normalizeRuntimeAttachments([
    { name: "note.txt", mime: "text/plain", size: 5, filePath: source }
  ]), cache);
