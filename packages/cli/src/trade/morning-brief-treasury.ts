...
export async function fetchTreasurySection(): Promise<string> {
  ...
  const priceEntries = Object.entries(cfg.price_watchlist);
  const yieldEntries = Object.entries(cfg.yield_watchlist);

  const priceResults = await Promise.all(
    priceEntries.map(async ([symbol, name]) => ({
      symbol,
      name,
      quote: await fetchTreasuryPriceQuote(symbol),
    })),
  );
  for (const { symbol, name, quote } of priceResults) {
  ...
  const yieldResults = await Promise.all(
    yieldEntries.map(async ([symbol, name]) => {
      ...
      return { symbol, name, quote, highCtx };
    }),
  );
  for (const { symbol, name, quote, highCtx } of yieldResults) {
