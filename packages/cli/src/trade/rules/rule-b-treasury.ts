...
function yieldSpikeDetail(
  ...
  yieldPct: number,
  changeBps: number,
  highCtx?: { lookback_years: number; period_high_pct: number; is_new_high: boolean; is_near_high: boolean; bps_below_high: number }
): string {
  const lines = [
  ...
  if (highCtx?.is_new_high) {
    ...
    lines.push(`逼近近 ${highCtx.lookback_years} 年高点 ${formatYieldPct(highCtx.period_high_pct)}（仅差 ${Math.abs(highCtx.bps_below_high).toFixed(1)}bp）。`);
  }
  ...
  return {
    ...
    async check(state: AlertState): Promise<Alert[]> {
      ...
      for (const [symbol, name] of Object.entries(cfg.yield_watchlist)) {
        ...
        const highCtx = periodHigh == null
          ? null
          : buildYieldHighContext(symbol, quote.yield_pct, periodHigh, cfg.yield_high_lookback_years, cfg.yield_near_high_bps);

        const highSeverity = highCtx?.is_new_high
          ? "critical"
