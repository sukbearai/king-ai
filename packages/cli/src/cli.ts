...
import {
  buildUsageRuntimeData,
  formatProviderCapabilities,
  formatRuntimeResultsTable,
  writeUsageRuntimeData,
...
const usageCommand = command(
  {
    ...
    help: {
      description:
        "Summarize local agent run usage from the running daemon state, list expenses, or export runtime data",
    },
  },
  async (argv) => {
...
const teamCommand = command(
  ...
  async (argv) => {
    ...
    if (
      scenarioId !== "repo-takeover" &&
      scenarioId !== "bug-investigation" &&
      scenarioId !== "product-design" &&
      scenarioId !== "release-check" &&
      scenarioId !== "research-brief"
    ) {
      throw new Error(
        "team scenario must be repo-takeover, bug-investigation, product-design, release-check, or research-brief",
      );
    }
...
export async function materializeTeamScenario(
  scenario: KingScenarioTemplate,
  outputDir: string,
  actorRole?: string,
): Promise<{ scenario: string; outputDir: string; cards: unknown[] }> {
  const cards: unknown[] = [];
...
const hostRemoteProfileCommand = command(
  ...
  async (argv) => {
    await printHostCommandResult(
      { command: "remote-profile", input: remoteInput(argv.flags) },
      Boolean(argv.flags.json),
    );
  },
);
...
const hostRemoteRunCommand = command(
  ...
  async (argv) => {
    ...
    await printHostCommandResult(
      { command: "remote-run", input: { ...remoteInput(argv.flags), cmd: argv.flags.cmd } },
      Boolean(argv.flags.json),
    );
  },
);
...
const hostRemoteLogsCommand = command(
  ...
  async (argv) => {
    await printHostCommandResult(
      {
        command: "remote-logs",
        input: { ...remoteInput(argv.flags), app: argv.flags.app, path: argv.flags.path, tail: argv.flags.tail },
      },
      Boolean(argv.flags.json),
    );
  },
    ...
    await printHostCommandResult(
      {
        ...
        input: {
          ...remoteInput(argv.flags),
          app: argv.flags.app,
          path: argv.flags.path,
          pattern: argv.flags.pattern,
          since: argv.flags.since,
...
const hostRemotePgCommand = command(
  ...
  async (argv) => {
    ...
    await printHostCommandResult(
      { command: "remote-pg", input: { ...remoteInput(argv.flags), db: argv.flags.db, sql: argv.flags.sql } },
      Boolean(argv.flags.json),
    );
  },
);
...
const hostRemoteRedisCommand = command(
  ...
  async (argv) => {
    if (!argv.flags.cmd) throw new Error("--cmd is required");
    await printHostCommandResult(
      { command: "remote-redis", input: { ...remoteInput(argv.flags), name: argv.flags.name, cmd: argv.flags.cmd } },
      Boolean(argv.flags.json),
    );
  },
);
...
const hostWorkflowCommand = command(
  ...
  async (argv) => {
    ...
    const commandName =
      action === "create" || action === "list" || action === "update"
        ? `workflow-${action}`
        : action === "initiative" ||
            action === "handoff" ||
            action === "review" ||
    ...
    if (!commandName)
      throw new Error(
        "workflow action must be create, list, update, initiative, handoff, review, decision, or artifact",
      );
    const result = await runHostCommandFromCli({
      command: commandName,
      format: argv.flags.json ? "json" : "text",
...
const tradeBriefCommand = command(
  {
    ...
    flags: {
      ...
      sections: {
        type: String,
        description: "Comma-separated sections: market,stocks,treasury,telegram,twitter,leaderboard,pumpfun",
      },
...
const tradeVerifyCelebrityCommand = command(
  {
    ...
    help: {
      description: "Check celebrity Twitter/X search pages for articles, no-results, login, or challenge states",
    },
  },
  async (argv) => {
    ...
    await cli(
      {
        ...
        commands: [
          ...
          hostPrepareRunLayoutCommand,
          hostSubmitRunCommand,
          hostRunRequestsCommand,
          hostRunRequestCommand,
          hostUpdateRunCommand,
          hostCancelRunCommand,
          hostExecuteRunCommand,
          hostEmitRunEventCommand,
          hostWatchRunCommand,
          hostRunResultsCommand,
          hostRunHeartbeatCommand,
          hostRunMetaCommand,
          hostPlanExportCommand,
          hostExportCommand,
          hostTimelineCommand,
          hostPolicyCommand,
          hostRemoteListCommand,
          hostRemoteProbeCommand,
          hostRemoteProfileCommand,
          hostRemoteRunCommand,
          hostRemoteLogsCommand,
          hostRemoteFindLogsCommand,
          hostRemotePgCommand,
          hostRemoteRedisCommand,
          hostWorkflowCommand,
        ],
        help: {
...
if (isDirectCliInvocation(process.argv[1], import.meta.url)) {
  void main().catch((err) => {
    process.stderr.write(
      `${commandNameFromArgv(process.argv[1])}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    process.exit(70);
  });
}
