...
Multi-role collaboration logic is part of the core product. `packages/cli/src/team-workflow.ts` defines role templates, team specs, permissions, and built-in workflow scenarios; `packages/cli/src/workflow-core.ts` is the canonical workflow engine (card shapes, status transitions, readiness, handoff planning, capability-first owner selection) shared by both the CLI host and the GUI worker, and `packages/cli/src/team-routing.ts` re-exports it under routing-friendly names; `packages/cli/src/host-control.ts` applies host command governance and workflow materialization. When changing agent/runtime behavior, preserve the distinction between the local execution boundary and the collaboration governance layer.

Keep the two collaboration layers consistent. Role templates in `team-workflow.ts` define capabilities and permissions; concrete agent rosters (e.g. the GUI `DEFAULT_TEAM_AGENTS`) may fold a template into another agent rather than mapping 1:1 — the GUI roster has no standalone `summarizer`, so the planner/`king-ai-ceo` owns loop-closing. Every built-in agent should declare its template with a `Role template: <id>.` marker in its role text so `roleTemplateForAgent` never has to guess. Tag by what the agent does, not who it talks to: in a single-agent conversation a `planner` coordinator answers in chat without creating a task, while a worker template (e.g. `builder`) self-delegates a task — so a single agent that performs the work directly (e.g. the IELTS coach) uses `builder`, not `planner`.
