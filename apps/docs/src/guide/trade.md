...
The Twitter brief applies a relevance filter by default. Its section title reports the cache/filter/analyzed funnel. In LLM mode, every filtered tweet up to `data_sources.twitter.max_display` (default `500`) is included in the summary input; candidates rank by available likes, retweets, replies, and views, with newer tweets winning ties. The summary emits at most five items and keeps a source index with author, UTC+8 time, and original URL. Set `data_sources.twitter.relevance_filter` to `false` to inspect the raw timeline. Telegram meme summaries prioritize priced buys/sells, liquidity, market cap, and concentration; transfer or airdrop lists are compressed and hard-capped, while referenced Chain.fm token contracts and abbreviated wallet addresses are emitted in full in a deterministic address index. Dry-run briefs do not replace the persisted metadata for the latest scheduled or manually delivered brief.
