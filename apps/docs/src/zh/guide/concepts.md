...
在 IELTS Study 工作流中，coach 会内联标注英文：每个句子用一个 `[core: ...]` 标出句中逐字连续出现的简短主语/动词片段，用 `[phrase: ...]` 只标有用的短词组。coach 应高亮句子里已有的文字，而不是另写一段压缩重述。当学习者输入中文时，可见的 Tip 行会补充这条中文请求对应的自然英语表达，并用中文解释高亮词组；隐藏的 Glossary 行只为单个内容词提供中文义、音标和音节拆分，让词卡有内容，同时不把原始词表显示给学习者。
