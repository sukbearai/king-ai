...
在 IELTS Study 工作流中，coach 会内联标注英文：每个句子用一个 `[core: ...]` 标出句中逐字连续出现的简短主语/动词片段，用 `[phrase: ...]` 只标有用的短词组。coach 应高亮句子里已有的文字，而不是另写一段压缩重述。隐藏的 Glossary 行会提供内容词的中文义、音标、音节拆分，以及高亮词组的中文义，让词卡和词组卡有内容，同时不把原始词表显示给学习者。
