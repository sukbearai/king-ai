...
在 IELTS Study 工作流中，coach 会内联标注英文：每个句子用一个 `[core: ...]` 标出句中逐字连续出现的简短主语/动词片段，用 `[phrase: ...]` 只标有用的短词组。coach 应高亮句子里已有的文字，而不是另写一段压缩重述。当学习者输入中文时，可见的 Tip 行会补充这条中文请求对应的自然英语表达，并用中文解释高亮词组。每个英文单词都可点击：隐藏的 `WordCards:` JSON 块必须为每个可见英文单词 token 提供结构化中文义、音标和音节拆分。runtime 会在接受 IELTS coach 回复前校验这个合同；如果 `WordCards` 不完整，会把缺失 token 返回给 agent，让它重试，而不是把空词卡展示给学习者。
