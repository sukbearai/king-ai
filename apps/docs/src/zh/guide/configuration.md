...
带结构化回复合同的 agent 会通过各引擎的原生接口传递 JSON Schema。Codex app-server turn 使用 `turn/start.outputSchema`；Codex one-shot 则把 schema 写入私有文件，并传递 `codex exec --output-schema <file>`。Claude 使用可续接的 one-shot print 模式和 `--output-format json --json-schema <schema>`，因为它的持久 stream 不接受每 turn schema。Grok 会切换为 `--output-format json --json-schema <schema>` 并读取返回的 `structuredOutput`；普通 Grok turn 仍使用 `streaming-json`。
...
Grok 通过 xAI CLI 的 headless 模式（`grok -p`）运行普通 turn，使用 `--output-format streaming-json`，并通过 `--resume <sessionId>` 复用 session。当 turn 带有已接受的图片附件时，King AI 会改用 `grok --prompt-json`，以 ACP 图片内容块（base64 载荷）发送，而不是纯文本 `-p`。脚本化运行时 King AI 还会传入 `--no-auto-update` 和 `--always-approve`。可通过 `KING_AI_GROK_ARGS` 追加可选参数。
