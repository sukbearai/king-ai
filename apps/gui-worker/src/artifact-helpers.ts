...
function hasMetadataDate(metadata: Record<string, unknown>): boolean {
  return (
    typeof metadata.collected_at === "string" ||
...
function requiresUnits(kind: string, pathValue: string, metadata: Record<string, unknown>): boolean {
  ...
  return (
    typeof metadata.currency !== "string" && typeof metadata.unit !== "string" && typeof metadata.period !== "string"
  );
}
...
export function checkArtifactQuality(artifact: ArtifactCandidate): ArtifactQualityCheck {
  ...
  if (!isKnownArtifactSource(artifact.source))
    warnings.push(`source is not a recognized identifier: ${artifact.source}`);
  if (artifact.source === "training_data" && artifact.confidence > 0.3)
    warnings.push("training_data confidence should be <= 0.3");
  if (artifact.source === "estimate" && artifact.confidence > 0.5)
    warnings.push("estimate confidence should be <= 0.5");
  if (
    artifact.source.startsWith("web_search:") &&
    artifact.confidence > 0.7 &&
    artifact.metadata.source !== "cross_validated"
  ) {
  ...
  if (requiresUnits(artifact.kind, artifact.path, artifact.metadata))
    warnings.push("metadata should include units such as currency, period, or unit");
...
export function formatArtifactLine(
  artifact: Pick<ArtifactLike, "id" | "kind" | "path" | "source" | "confidence" | "taskId" | "verified">,
): string {
