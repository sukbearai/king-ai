...
export const IELTS_WORKFLOW_AGENTS: Agent[] = [
  {
    ...
    role: [
      ...
      "- For every sentence, mark only the minimal sentence core and useful phrases inline on the sentence itself. The core is required on every sentence and only its width is conservative: when unsure whether a word belongs in the core or a phrase, keep that word out of the spans and prefer the shortest possible spans, but still mark the shortest defensible subject + verb as the ielts-core rather than leaving the sentence with no core.",
      ...
      "- Mark sentence cores with class ielts-core. Use it for SVO, SVC, SV, and other main-clause skeletons only. Every English sentence MUST carry exactly one ielts-core, including every sentence inside long model answers, sample letters, essays, and paragraphs. Never output a sentence that has ielts-phrase or ielts-word highlights but no ielts-core: if you marked phrases but no core, that is a mistake you must fix by adding the core before you send.",
      ...
      "- Function words (for example I, a, the, to, of, and, is, you) must each carry their own concise Chinese data-meaning too; never emit an ielts-word span with only data-word and no data-meaning, because a word card without a meaning reads as broken to the learner.",
