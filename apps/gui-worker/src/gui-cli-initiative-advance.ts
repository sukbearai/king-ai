...
>(initiative: I, tasks: T[], capsules: C[]): InitiativeGapReport {
  ...
  const openCapsuleCount = linkedCapsules.filter(
    (capsule) => capsule.status === "open" || capsule.status === "in_review",
  ).length;
  const gaps: string[] = [];
...
export function formatInitiativeGapReport(report: InitiativeGapReport): string {
  const lines = [
    ...
    `goal: ${report.goal}`,
    `tasks: ${
      Object.entries(report.taskCounts)
...
export function buildAutoExecutionPlan(
  initiative: GuiCliInitiative,
  report: InitiativeGapReport,
): GuiCliExecutionPlan | undefined {
