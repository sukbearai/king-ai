...
type CorrectionRequestStatus = "pending" | "done" | "failed";
...
const CORRECTION_REQUEST_CAPACITY = 50;
const CORRECTION_RPC_TIMEOUT_MS = 3500;
...
app.get("/runtime/corrections/:id", async (c) => await (await stateForRequest(c)).fetch(`https://state/corrections/${encodeURIComponent(c.req.param("id"))}`, {
  headers: await forwardHeaders(c)
}));

app.post("/runtime/corrections/:id/complete", async (c) => await (await stateForRequest(c)).fetch(`https://state/corrections/${encodeURIComponent(c.req.param("id"))}/complete`, {
  method: "POST",
...
app.post("/gui/correct-message", async (c) => {
  const blocked = await requireGuiAuth(c);
  if (blocked) return blocked;
  return (await stateForRequest(c)).fetch("https://state/gui-correct-message", {
  method: "POST",
  headers: await forwardHeaders(c, { "Content-Type": "application/json" }),
  body: JSON.stringify(await c.req.json().catch(() => ({})))
  });
...
export class GuiState implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path.startsWith("/corrections/") && path.endsWith("/complete")) return this.authRuntime(request, async (agentId) => this.completeCorrection(path.split("/")[2] || "", await request.json().catch(() => ({})), agentId));
    if (path.startsWith("/corrections/")) return this.authRuntime(request, async (agentId) => this.correctionPayload(path.split("/")[2] || "", agentId));
    ...
    if (path === "/gui-message") return this.guiMessage(request, await request.json() as { body?: string; conversationId?: string; correctionRequestId?: string });
    if (path === "/gui-correct-message") return this.guiCorrectMessage(await request.json().catch(() => ({})) as { body?: string; conversationId?: string });
  ...
  private normalizeState(saved: State): State {
    ...
    saved.correctionRequests = normalizeCorrectionRequests(saved.correctionRequests ?? []);
  ...
  private async correctionPayload(requestIdRaw: string, agentId: string): Promise<Response> {
    const requestId = decodeURIComponent(requestIdRaw || "");
    const state = await this.get();
    const request = state.correctionRequests.find((row) => row.id === requestId);
    if (!request) return json({ error: "correction request not found" }, { status: 404 });
    if (request.requesterAgentId !== agentId) return json({ error: "correction request not assigned to this agent" }, { status: 403 });
    return json({
      id: request.id,
      body: request.body,
      conversationId: request.conversationId,
      instructions: [
        "Correct obvious typos, missing characters, and accidental wording errors in this chat message before it is sent.",
        "Preserve code, commands, paths, URLs, identifiers, quoted text, numbers, and formatting exactly unless the user explicitly asks to edit them.",
        "If the intended correction is uncertain or would change meaning, return the original message.",
        "Return strict JSON only: {\"corrected\":\"...\"}."
      ].join("\n")
    });
  }

  private applyCorrectionToBoundMessage(state: State, request: CorrectionRequest): void {
    const corrected = request.corrected?.trim();
    if (!corrected || !request.messageId || !request.conversationId) return;
    const message = state.messages.find((row) =>
      row.id === request.messageId &&
      row.conversation_id === request.conversationId &&
      row.author_kind === "human"
    );
    if (!message || message.body === corrected) return;
    message.body = corrected;
    delete message.body_html;
    const conversation = state.conversations.find((row) => row.id === request.conversationId);
    if (conversation) conversation.updated_at = Math.max(conversation.updated_at, Date.now());
  }

  private async completeCorrection(requestIdRaw: string, payload: unknown, agentId: string): Promise<Response> {
    const requestId = decodeURIComponent(requestIdRaw || "");
    const state = await this.get();
    const request = state.correctionRequests.find((row) => row.id === requestId);
    if (!request) return json({ error: "correction request not found" }, { status: 404 });
    if (request.requesterAgentId !== agentId) return json({ error: "correction request not assigned to this agent" }, { status: 403 });
    const body = payload && typeof payload === "object" ? payload as { corrected?: unknown; error?: unknown } : {};
    const corrected = typeof body.corrected === "string" ? body.corrected.trim() : "";
    request.status = corrected ? "done" : "failed";
    request.corrected = corrected || request.body;
    request.error = typeof body.error === "string" ? body.error.slice(0, 500) : undefined;
    request.updatedAt = Date.now();
    this.applyCorrectionToBoundMessage(state, request);
    state.correctionRequests = normalizeCorrectionRequests(state.correctionRequests);
    await this.put(state);
  ...
  private async guiMessage(request: Request, payload: { body?: string; conversationId?: string; correctionRequestId?: string }): Promise<Response> {
    const state = await this.get();
    ...
    const correctionRequestId = typeof payload.correctionRequestId === "string" ? payload.correctionRequestId.trim() : "";
    ...
    const correction = correctionRequestId
      ? state.correctionRequests.find((row) =>
        row.id === correctionRequestId &&
        row.conversationId === conversation.id &&
        (row.body === message.body || row.corrected?.trim() === message.body)
      )
      : undefined;
    if (correction) {
      correction.messageId = message.id;
      correction.updatedAt = now;
      message.correctionRequestId = correction.id;
      if (correction.status === "done" && correction.corrected?.trim()) {
        message.body = correction.corrected.trim();
      }
    }
  ...
  private async guiCorrectMessage(payload: { body?: string; conversationId?: string }): Promise<Response> {
    const state = await this.get();
    const body = typeof payload.body === "string" ? payload.body.trim() : "";
    if (!body) return json({ corrected: "", changed: false, fallback: false });
    const conversationId = typeof payload.conversationId === "string" && payload.conversationId.trim()
      ? payload.conversationId.trim()
      : DEFAULT_CONVERSATION.id;
    const conversation = ensureConversation(state, conversationId);
    const coordinator = coordinatorAgentFor(state, conversation);
    const now = Date.now();
    const runtimeOnline = Boolean(state.availableEngines.length && state.lastHeartbeat?.at && now - state.lastHeartbeat.at < 90_000);
    if (!runtimeOnline) {
      return json({ corrected: body, changed: false, fallback: true, error: "correction runtime unavailable" });
    }
    const request: CorrectionRequest = {
      id: `correction-${now}-${Math.random().toString(36).slice(2)}`,
      body,
      conversationId: conversation.id,
      requesterAgentId: coordinator.id,
      status: "pending",
      createdAt: now,
      updatedAt: now
    };
    state.correctionRequests.push(request);
    state.correctionRequests = normalizeCorrectionRequests(state.correctionRequests);
    await this.put(state);
    await this.broadcast({ event: "correction", data: { correctionRequestId: request.id, agentId: coordinator.id, conversationId: conversation.id, at: now } });

    const startedAt = Date.now();
    while (Date.now() - startedAt < CORRECTION_RPC_TIMEOUT_MS) {
      await sleep(100);
      const latestState = await this.get();
      const latest = latestState.correctionRequests.find((row) => row.id === request.id);
      if (latest?.status === "done") {
        const corrected = latest.corrected?.trim() || body;
        return json({ corrected, changed: corrected !== body, fallback: false, correctionRequestId: request.id });
      }
      if (latest?.status === "failed") {
        return json({ corrected: body, changed: false, fallback: true, error: latest.error, correctionRequestId: request.id });
      }
    }
    return json({ corrected: body, changed: false, fallback: true, error: "correction timed out", correctionRequestId: request.id });
  }
...
function normalizeCorrectionRequests(requests: CorrectionRequest[]): CorrectionRequest[] {
  return requests
    .filter((request) => request && typeof request.id === "string" && typeof request.body === "string")
    .map((request) => {
      const status: CorrectionRequestStatus = request.status === "done" || request.status === "failed" ? request.status : "pending";
      return {
        id: request.id,
        body: request.body,
        conversationId: typeof request.conversationId === "string" ? request.conversationId : undefined,
        messageId: typeof request.messageId === "string" ? request.messageId : undefined,
        requesterAgentId: normalizeAgentId(request.requesterAgentId) ?? DEFAULT_AGENT.id,
        status,
        corrected: typeof request.corrected === "string" ? request.corrected : undefined,
        ...
        updatedAt: typeof request.updatedAt === "number" ? request.updatedAt : Date.now()
      };
    })
    .slice(-CORRECTION_REQUEST_CAPACITY);
}
